...
void main() {
  group('CosyVoice2 mel parity', () {
    ...
    if (!fixture.existsSync()) {
      throw StateError(
        'Missing mel_cases.json fixture at ${fixture.path}',
      );
    }
    ...
    for (final raw in cases) {
      ...
      test('$name ($kind)', () {
        final audio =
            Float32List.fromList((c['audio'] as List).cast<num>().map((e) => e.toDouble()).toList());
        ...
        expect(got.numMels, expectedShape[0],
            reason: 'numMels mismatch for $name');
        expect(got.nFrames, expectedShape[1],
