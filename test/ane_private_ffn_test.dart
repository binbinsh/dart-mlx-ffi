...
void _expectClose(Float32List actual, Float32List expected, String name) {
  ...
  for (var index = 0; index < actual.length; index++) {
    expect(actual[index], closeTo(expected[index], 2e-2), reason: name);
  }
}

void main() {
  test('private ane benchmarks ffn blocks under latency budgets', () async {
    final result = await Process.run('uv', [
      'run',
      'python',
      'private_ane/shared/benchmark/ane_private_ffn_bench.py',
      '--json',
      '--iters',
      '10',
    ]);
  ...
  test('private ane dart api runs ffn blocks', () async {
    ...
    final tempDir = await Directory.systemTemp.createTemp('ane_private_ffn_');
    addTearDown(() => tempDir.delete(recursive: true));

    final gen = await Process.run('uv', [
      'run',
      'python',
      'private_ane/shared/benchmark/ane_private_make_ffn_models.py',
      '--out-dir',
      tempDir.path,
    ]);
