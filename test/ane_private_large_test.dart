...
void _expectClose(Float32List actual, Float32List expected, String name) {
  ...
  for (var index = 0; index < actual.length; index++) {
    expect(actual[index], closeTo(expected[index], 1e-3), reason: name);
  }
}

void main() {
  test('private ane benchmarks larger mlprogram models under latency budgets', () async {
    final result = await Process.run('uv', [
      'run',
      'python',
      'private_ane/shared/benchmark/ane_private_large_bench.py',
      '--json',
      '--iters',
      '10',
    ]);
  ...
  test('private ane dart api runs larger mlprogram models', () async {
    ...
    final gen = await Process.run('uv', [
      'run',
      'python',
      'private_ane/shared/benchmark/ane_private_make_large_models.py',
      '--out-dir',
      tempDir.path,
    ]);
    ...
    for (final spec in models) {
      ...
      try {
        ...
        try {
          ...
          _expectClose(outputs.first, input, name);
        } finally {
          session.close();
        }
      } finally {
