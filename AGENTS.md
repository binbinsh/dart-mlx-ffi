...
- The canonical release form is `1.2026.34` where:
  - `1` is the fixed package major version
  - `2026` is the 4-digit calendar year
  - `34` is the git commit count with no zero padding
- Update [`pubspec.yaml`](pubspec.yaml) and [`CHANGELOG.md`](CHANGELOG.md) together.
- Git tags must match the pubspec version and use the form `v<version>`, Example tag: `v1.2026.34`.
