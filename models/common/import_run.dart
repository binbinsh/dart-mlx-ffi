...
void main(List<String> args) {
  if (args.length != 2 && args.length != 3) {
    stderr.writeln(
      'usage: dart run models/common/import_run.dart <export_path> <input_safetensors_path> [input_names_json]',
    );
    exitCode = 64;
    return;
  }
  ...
  final warmup =
      int.tryParse(Platform.environment['GENERIC_WARMUP'] ?? '') ?? 0;
  ...
  try {
    final runOnce = _selectRunner(imported, orderedInputs, stream);
...
MlxArray Function() _selectRunner(
  MlxImportedFunction imported,
  List<MlxArray> inputs,
  MlxStream stream,
) {
  try {
    final probe = _runOnceFast(imported, inputs, stream);
    probe.close();
    return () => _runOnceFast(imported, inputs, stream);
  } on Object {
    return () => _runOnceSlow(imported, inputs, stream);
  }
}

MlxArray _runOnceFast(
  MlxImportedFunction imported,
  List<MlxArray> inputs,
  MlxStream stream,
) {
  final out = imported.callOne(inputs);
  MlxRuntime.asyncEvalAll([out]);
  stream.synchronize();
  return out;
}

MlxArray _runOnceSlow(
  MlxImportedFunction imported,
  List<MlxArray> inputs,
  MlxStream stream,
) {
