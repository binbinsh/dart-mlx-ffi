...
extension on Qwen3_5Runner {
  ...
  MlxArray _fullAttention(
    ...
    attn.close();
    final attnPost = _privateAneAttnPost;
    if (attnPost != null &&
        layerIndex != null &&
        attnPost.hasLayer(layerIndex)) {
      final out = attnPost.runLayer(layerIndex, merged, gate, seqLen);
      gate.close();
      merged.close();
      if (traceFull) {
        stderr.writeln(
          'qwen35_run: full projected ${_previewLastSeq(out)}',
        );
      }
      return out;
    }
  ...
  MlxArray _denseMlp(
    ...
    final ane = _privateAneMlp;
    if (ane != null && layerIndex != null && ane.hasLayer(layerIndex)) {
      return ane.runLayer(layerIndex, input, seqLen);
    }
    final wantedLayer = int.tryParse(
      Platform.environment['QWEN35_TRACE_LAYER'] ?? '',
    );
