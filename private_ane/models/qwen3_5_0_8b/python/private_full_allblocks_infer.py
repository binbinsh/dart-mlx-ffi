...
try:
    ...
    from .private_linear_runtime import PrivateLinearLayerRuntime
...
def main() -> None:
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="qwen35_private_allblocks_"))
    try:
        ...
        try:
            ...
            for index, layer in enumerate(model.language_model.model.layers):
                if layer.is_linear:
                    linear_runtimes[index] = PrivateLinearLayerRuntime.build(
                        layer.linear_attn,
                        lane=args.lane,
                    )
                ...
                for layer_index, layer in enumerate(base.layers):
                    if layer.is_linear:
                        ...
                        if layer_index not in attn_layers:
                            r = layer.self_attn(x_norm, fa_mask, None, position_ids)
                        else:
                            bsz, seq_len, _dim = x_norm.shape
                            q_proj_output = sa.q_proj(x_norm)
                            queries, gate = mx.split(
                                q_proj_output.reshape(
                                    bsz, seq_len, sa.num_attention_heads, -1
                                ),
                                2,
                                axis=-1,
                            )
                            ...
                            if layer_index in attn_post_runtimes:
                                r = attn_post_runtimes[layer_index].run(
                                    ctx_np=ctx_all.astype(np.float32, copy=False),
                                    gate_np=to_numpy_f32(gate),
                                )
                            else:
                                ctx_mx = mx.array(
                                    ctx_all.astype(np.float16), dtype=mx.float16
                                )
                                r = sa.o_proj(
                                    ctx_mx.transpose(0, 2, 1, 3).reshape(
                                        bsz, seq_len, -1
                                    )
                                    * mx.sigmoid(gate)
                                )
            ...
            all_tokens, all_ms = generate_greedy(
                combo_next_logits,
                token_ids,
                args.max_new_tokens,
                tokenizer.eos_token_id,
            ...
            for runtime in linear_runtimes.values():
                runtime.close()

        report = {
            "runtime": "qwen35_private_full_allblocks_hybrid",
            "model_id": MODEL_ID,
            ...
            "allblocks_text": tokenizer.decode(
                all_tokens,
                skip_special_tokens=False,
            ),
