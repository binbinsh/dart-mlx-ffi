...
from make_private_blocks import (
    ...
    CONFIG_URL,
    build_specs,
    compute_audio_expected,
    compute_text_expected,
    emit_block,
...
def bench_mlx_block(spec: dict[str, object], *, warmup: int, iters: int) -> dict[str, object]:
    ...
    if spec["block_type"] == "audio_mlp":
        w1_np, w2_np = make_audio_weights(spec)
        expected = compute_audio_expected(sample, w1=w1_np, w2=w2_np)
        ...
        w1_np, w3_np, w2_np = make_text_weights(spec)
        expected = compute_text_expected(sample, w1=w1_np, w3=w3_np, w2=w2_np)
        w1 = mx.array(w1_np[:, :, 0, 0], dtype=mx.float32)
    ...
    return {
        ...
        "output_preview": values.reshape(-1)[:8].astype(np.float32).tolist(),
    }
...
def main() -> None:
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="qwen3_asr_compare_"))
    try:
        ...
        models.append(
            {
                ...
                "mlx_output_preview": mlx_model["output_preview"],
            }
        )

    report = {
        "runtime": "qwen3_asr_private_vs_mlx",
    ...
    print("qwen3-asr private ane vs mlx")
