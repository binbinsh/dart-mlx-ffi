...
def main() -> None:
    ...
    q = rng.normal(0.0, 0.1, size=(1, num_heads, args.seq_len, head_dim)).astype(np.float16)
    k_small = rng.normal(0.0, 0.1, size=(1, kv_heads, args.seq_len, head_dim)).astype(np.float16)
    v_small = rng.normal(0.0, 0.1, size=(1, kv_heads, args.seq_len, head_dim)).astype(np.float16)
    k = np.repeat(k_small, repeat, axis=1)
    v = np.repeat(v_small, repeat, axis=1)
    scale = head_dim ** -0.5

    q_mx = mx.array(q, dtype=mx.float16)
    k_mx = mx.array(k, dtype=mx.float16)
    v_mx = mx.array(v, dtype=mx.float16)
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="josie_sdpa_"))
    try:
        root = work_dir / "artifact"
        root.mkdir(parents=True, exist_ok=True)
        (root / "q.bin").write_bytes(q.tobytes())
        (root / "k.bin").write_bytes(k.tobytes())
        (root / "v.bin").write_bytes(v.tobytes())
        ...
        ane_out = np.frombuffer(out_path.read_bytes(), dtype=np.float16).astype(np.float32)
        ane_out = ane_out.reshape(1, num_heads, args.seq_len, head_dim)
        diffs = np.abs(ane_out - mlx_out)
        report = {
            "runtime": "josie_sdpa_private_vs_mlx",
            "seq_len": args.seq_len,
            "num_heads": num_heads,
            "num_key_value_heads": kv_heads,
            "head_dim": head_dim,
            "ane_per_iter_ms": float(payload["per_iter_ms"]),
            "mlx_per_iter_ms": mlx_ms,
            "ane_speedup_vs_mlx": mlx_ms / float(payload["per_iter_ms"]),
        ...
        if args.json:
            ...
            return
        print("josie sdpa core private ane vs mlx")
