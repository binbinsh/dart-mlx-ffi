...
and Android. Linux remains an optional runtime target for local/server
validation, but it is outside the production promotion gate. Windows is listed
under `support_policy.abandoned_platforms` and is excluded from default runtime
matrix, audit, and promotion flows.
...
The default `run_all.py` plan uses the production platform policy from
`models.yaml`: `ios`, `macos`, and `android`. Its summary separates runtime
readiness from production evidence:

- `readiness_counts`: the primary four-way matrix summary with
  `production-ready`, `preferred-ready`, `fallback-ready`, and `blocked`
  buckets.
- `artifact_ready_count`: cells that can execute through either the preferred
  engine or a fallback engine. This is artifact/runtime availability, not
  production promotion.
- `model_readiness_counts`: the same four-way classification aggregated by
  model, where a model is `production-ready` only when every planned platform
  cell is production-ready.
- `production_model_count`: model-level production-ready count.
- `production_ready_count`: preferred engine is selected and the platform
  verdict already passes every promotion gate.
- `preferred_ready_count`: the preferred engine cell can run, but promotion
  evidence is still missing or incomplete.
- `fallback_ready_count`: the cell can run only through a fallback engine, such
  as Apple MLX while Core ML is still blocked.
- `blocked_count`: no acceptable preferred or fallback execution path is
  available for that production platform.

`ready_count` is retained for older scripts and is equivalent to
`artifact_ready_count`; do not read it as production-ready.

Android defaults to LiteRT, but model entries may override `engine_order` when
ONNX Runtime Mobile is the correct production runtime for the model shape. This
is used for componentized VLM/audio/TTS models whose current graphs rely on ORT
features or control-flow forms that LiteRT cannot load correctly yet.

Explicit Windows planning is rejected by default because Windows is abandoned.
Use `--include-abandoned-platforms` only for historical diagnostics.
...
validation before the Android blocker can be removed. The ONNX export path is
kept as optional Linux/server tooling and no longer blocks production.
...
and Android. Linux artifacts may be kept in the catalog for optional
server-side validation, but Windows artifacts are not part of the active
support target.
...
and then overlays converted artifacts/blockers. This keeps existing Android
ONNX cells for models that explicitly prefer ONNX Runtime Mobile instead of
replacing the whole model entry.
...
catalog now makes Android prefer ONNX Runtime Mobile for this model because the
available ONNX component graph contains control-flow bindings that are invalid
for direct LiteRT conversion. Apple still tracks Core ML as preferred and uses
MLX as staging fallback where Core ML is missing. Its componentized ONNX
artifact should also pass
...
Linux and Android use the int4 component bundle with
...
Android now treats that ONNX path as the preferred runtime for Qwen3-ASR;
`benchmark/runtime/converters/qwen3_asr_onnx_to_litert.py` remains tracked as a
future LiteRT optimization path once chunked decoder or native MatMulNBits
support validates. `kitten_tts` is pinned to the mini
0.8 family and also prefers Android ONNX while `onnx2tf`
LiteRT attempts are tracked as non-preferred engine blockers
(`onnx2tf_attempt_timeout`) with
