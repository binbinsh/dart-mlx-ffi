...
def main() -> None:
    ...
    # Hybrid path needs the original (un-normalised, un-resized) RGB HWC
    # uint8 bytes plus the original height/width: it does its own
    # smart_resize + normalisation inside Dart so a parity comparison
    # exercises the full hybrid preprocessing chain.
    raw_rgb = np.array(image, dtype=np.uint8)
    assert raw_rgb.ndim == 3 and raw_rgb.shape[2] == 3, (
        f"unexpected raw image shape {raw_rgb.shape}"
    )
    ...
    np.save(args.out_dir / "image_rgb_hwc_u8.npy", raw_rgb)
        ...
        json.dumps(
            {
                ...
                "image_rgb_hwc_u8_path": str(args.out_dir / "image_rgb_hwc_u8.npy"),
                "image_orig_height": int(image.height),
                "image_orig_width": int(image.width),
