...
- Apple builds also link Zig to a private `dart_inference_mlx_c` library that
  ...
  safetensors artifact layouts, load safetensors weight maps on Apple, and
  `run()` enters Zig-side managed tensor-to-`mlx_array` conversion before
  returning an executor-not-implemented error.
