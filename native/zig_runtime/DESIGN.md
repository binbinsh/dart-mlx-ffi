...
- MLX calls move behind Zig. Until the `mlx-c` executor is implemented and
  ...
  sessions are created by Zig, validate local safetensors artifact layouts, load
  Apple `mlx-c` weight maps into Zig-owned session state, and execution must
  fail from the Zig-owned `mlx-c` path rather than the C/C++ adapter.
