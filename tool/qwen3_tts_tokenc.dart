...
Future<void> main(List<String> args) async {
  if (args.length < 2) {
    stderr.writeln(
      'usage: dart run tool/qwen3_tts_tokenc.dart <bundle_path> <wav_path>',
    );
    exitCode = 64;
    return;
  }
  ...
  try {
    final encoder = Qwen3TtsTokenizerEncoder(bundle);
    final samples = _readWavMonoFloat32(args[1]);
    final codes = encoder.encode(samples);
    stdout.writeln(
      jsonEncode(<String, Object?>{
        'groups': codes.length,
        'time': codes.isEmpty ? 0 : codes.first.length,
        'first3': codes.take(3).map((row) => row.take(16).toList()).toList(),
      }),
    );
  } finally {
    bundle.close();
