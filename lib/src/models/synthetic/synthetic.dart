...
Map<String, Object?> runSyntheticModelBenchmarks({
  required int warmup,
  required int iters,
}) {
...
void printSyntheticBenchmarkReport(Map<String, Object?> report) {
  stdout.writeln('dart_inference model benchmark');
  ...
  for (final model in (report['models'] as List<Object?>).cast<Map<String, Object?>>()) {
    ...
    stdout.writeln('  per_iter_ms: ${(model['per_iter_ms'] as double).toStringAsFixed(4)}');
    stdout.writeln('  peak_bytes_delta: ${model['peak_bytes_delta']}');
  }
}

int readSyntheticBenchArg(List<String> args, String name, {required int fallback}) {
  final prefix = '$name=';
...
Map<String, Object?> _benchAttention({
  ...
  try {
    MlxArray forward() {
      final attention = mx.fast.scaledDotProductAttention(q, k, v, scale: 0.25);
