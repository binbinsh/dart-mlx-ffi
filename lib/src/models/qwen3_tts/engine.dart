...
final class Qwen3TtsEngine {
  Qwen3TtsEngine._({
    ...
    required _Qwen3TtsTalker talker,
    required _Qwen3TtsSpeechDecoder decoder,
  }) : _talker = talker,
       _decoder = decoder,
       _tokenizer = Qwen3AsrBpeTokenizer.load(bundle.manifest.rootPath),
       _temperature = ((bundle.generationConfig['temperature'] as num?) ?? 0.9)
           .toDouble(),
       _topK = ((bundle.generationConfig['top_k'] as num?) ?? 50).toInt(),
       _repetitionPenalty =
           ((bundle.generationConfig['repetition_penalty'] as num?) ?? 1.05)
               .toDouble(),
       _streamingChunkSize = _resolveStreamingChunkSize(
         bundle.generationConfig,
       ),
       _suppressTokenIds = [
         for (
           var i = bundle.manifest.talker.vocabSize - 1024;
  ...
  static int _resolveStreamingChunkSize(Map<String, Object?> generationConfig) {
    final raw = ((generationConfig['streaming_interval'] as num?) ?? 0.5)
        .toDouble();
    final seconds = raw.isFinite ? math.max(0.25, raw) : 0.5;
    return math.max(4, (seconds * 12.5).round());
  }
  ...
  Stream<Qwen3TtsChunk> synthesiseStream(
    ...
    try {
      final maxTokens = math.min(
        4096,
        math.max(75, prefill.targetTokenCount * 6),
      );
        ...
        try {
          for (
            ...
            if (codeIdx == 0) {
              ...
              codeInput = _talker.embedPredictorToken(
                codeIdx - 1,
                codeTokens.last,
              );
            }
            final logits = _talker.forwardCodePredictor(
              codeInput,
              codeIdx,
              codeCache,
            );
          ...
          for (var index = 1; index < codeTokens.length; index++) {
            final extra = _talker.embedPredictorToken(
              index - 1,
              codeTokens[index],
            );
  ...
  _Qwen3TtsIclPrefill _prepareIclInputs(
    String text,
    Qwen3TtsPreparedReference reference,
  ) {
    ...
    if (refIds.length < 5 || targetIds.length < 8) {
      ...
      refCodes.close();
      throw StateError(
        'Qwen3-TTS prompt templates encoded unexpectedly short token sequences.',
      );
    }
    ...
    final ttsBos = ttsEmbeds.slice(
      start: [0, 0, 0],
      stop: [1, 1, cfg.hiddenSize],
    );
    final ttsEos = ttsEmbeds.slice(
      start: [0, 1, 0],
      stop: [1, 2, cfg.hiddenSize],
    );
    final ttsPad = ttsEmbeds.slice(
      start: [0, 2, 0],
      stop: [1, 3, cfg.hiddenSize],
    );
    ttsEmbeds.close();

    final combinedTextIds = <int>[
      ...refIds.sublist(3, refIds.length - 2),
      ...targetIds.sublist(3, targetIds.length - 5),
    ];
    ...
    final firstCb = refCodes
        .slice(start: [0, 0, 0], stop: [1, 1, refTime])
        .reshape([1, refTime]);
    var refCodecEmbed = _talker.embedCodecToken(firstCb);
    firstCb.close();
    for (var index = 0; index < cfg.numCodeGroups - 1; index++) {
      final group = refCodes
          .slice(start: [0, index + 1, 0], stop: [1, index + 2, refTime])
          .reshape([1, refTime]);
    ...
    final codecPrefixSuffix = _talker.embedCodecIds([
      cfg.codecPadId,
      cfg.codecBosId,
    ]);
    final prefixWithSpeaker = mx.concatenate([
      codecPrefix,
      speakerEmbed,
      codecPrefixSuffix,
    ], axis: 1);
    ...
    final roleEmbed = _talker.projectText(
      _talker.embedTextIds(targetIds.take(3).toList(growable: false)),
    );
    ...
    final inputEmbeds = mx.concatenate([
      roleEmbed,
      combinedPrefix,
      iclInput,
    ], axis: 1);
